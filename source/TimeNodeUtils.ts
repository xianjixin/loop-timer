 ...
 * @param nowTime
 */
function isValidByHours(item: iLoopTimer, nowTime: Date): boolean {
 ...
 */
function isValidTime(time: string, lastTime: number, nowTime: number, reg: RegExp, type: eTimeType): boolean {
  if (new RegExp(/^\d+\,\d+$/).test(time)) {
    ...
    let intervalNum = Number(interval);
    //如果时间是秒和分钟  进制是60
    if (type === eTimeType.SECONDS || type === eTimeType.MINUTES) {
      if (lastTime > nowTime) {
